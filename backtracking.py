    ...
    for i, j in product([row for row in range(size)],
        ...
        for number in  domain:
            ...
            if(not cage_check(grid, cages, number_cages)):
                grid[i,j]=0
                continue
            is_solved, grid = solve_kenken(grid, cage_constraints, size, number_cages, domain)
            if is_solved:
                return True, grid
            grid[i, j] = 0
        return False, grid
...
if __name__ == "__main__":
    ...
    is_solved, solved = solve_kenken(grid, cage_constraints, size, number_cages, domain)
    if is_solved:
        print(solved)
    else:
        print("Cannot solve")